...
for Year in YearList:
    ...
    for FileName in FileList:
        ...
        if FileName == FileList[0]:
            AnnualData = pd.DataFrame({'DateTime': np.repeat(DS.time.values, DimGrid)})
            AnnualData["Year"] = AnnualData["DateTime"].dt.year
            AnnualData["Month"] = AnnualData["DateTime"].dt.month
            AnnualData["Day"] = AnnualData["DateTime"].dt.day
            AnnualData = AnnualData.drop(columns = ["DateTime"])
                
            AnnualData['Latitude'] = np.tile(Lat, DimTime),
            AnnualData['Longitude'] = np.tile(Lon , DimTime),
