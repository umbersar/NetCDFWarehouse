...
FORMAT 2:
The CSV files are written to include the spatial dimension as part of the fact table, while still
requiring joins to include the temporal dimensions. That is; Latitude and Longitude are included
in the fact table, alongside the DateID, but no actual temporal dimensions.  The fact tables contain
 annual data for multiple variables, alongisde the corresponding DateID, Latitude, and Longitude values.

'DIMENSION_DATETIME.csv'  : | DateID (int) | DateTime (datetime) |
'DIMENSION_YMD.csv'       : | DateID (int) |     Year (int)      |    Month (int)    |   Day (int)  |
'FACT_YYYY.csv'           : | DateID (int) |   Latitude (float)  | Longitude (float) | Var1 (float) |  ...  | VarN (float)|
...
OutputFolder = "D:\\CSIRO\\CSV FORMAT 2"
...
for Year in FileTable["Year"].drop_duplicates():
    ...
    for FileName in FileSeries:
        ...
        # For the first variable, a dataframe is created with DateID, Latitude, and Longitude columns.
        if FileName == FileSeries[0]:
            AnnualData = pd.DataFrame({
                "DateID": np.arange(DateID_Start, DateID_End + 1).repeat(DimGrid),
                "Latitude": np.tile(Latitude, DimTime),
