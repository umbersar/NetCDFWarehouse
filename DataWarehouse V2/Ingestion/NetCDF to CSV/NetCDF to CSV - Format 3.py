...
FORMAT 3:
The CSV files are written to just include a joining key to the separate temporal dimension table.
The fact table holds no explicit reference to location beyond its order. When inserted into SSMS
alongside the grid dimension tables, with order preserved, they will be able to joined by the
implicit identity column. This however requires a separate grid dimension table for both leap and
non-leap years. The fact tables contain annual data for multiple variables, alongisde the
 corresponding DateID column.

'DIMENSION_GRID_STANDARD.csv'   : | GridID (int) |   Latitude (float)  | Longitude (float) |
'DIMENSION_GRID_LEAP.csv'       : | GridID (int) |   Latitude (float)  | Longitude (float) |
'DIMENSION_DATETIME.csv'        : | DateID (int) | DateTime (datetime) |
'DIMENSION_YMD.csv'             : | DateID (int) |     Year (int)      |    Month (int)    |   Day (int)  |
'FACT_YYYY.csv'                 : | DateID (int) |     Var1 (float)    |        ...        | VarN (float)|
...
OutputFolder = "D:\\CSIRO\\CSV FORMAT 3"
...
2.1 GRID DIMENSION TABLE - STANDARD YEAR
...
DimTime = FileTable["Time Resolution"].min()
DF_Grid = pd.DataFrame({"LATITUDE": np.tile(Latitude, DimTime),
                        "LONGITUDE": np.tile(Longitude, DimTime)})
DF_Grid.index = DF_Grid.index + 1
DF_Grid.to_csv(((OutputFolder+"\\DIMENSION_GRID_STANDARD.csv")), index = True, header=True)

'''
2.2 GRID DIMENSION TABLE - LEAP YEAR
This will be a table with columns; |GridID (int)|Latitude (float)|Longitude (float)|
As confirmed above, all files have the same spatial dimensions, so this table can be extracted from any file.
'''
DimTime = FileTable["Time Resolution"].max()
DF_Grid = pd.DataFrame({"LATITUDE": np.tile(Latitude, DimTime),
                        "LONGITUDE": np.tile(Longitude, DimTime)})
DF_Grid.index = DF_Grid.index + 1
DF_Grid.to_csv(((OutputFolder+"\\DIMENSION_GRID_LEAP.csv")), index = True, header=True)


'''
2.3 WRITE DATETIME DIMENSION TABLE TO CSV
...
'''
2.4 WRITE YEAR-MONTH-DAY DIMENSION TABLE TO CSV
...
for Year in FileTable["Year"].drop_duplicates():
    ...
    for FileName in FileSeries:
        ...
        # For the first variable, a dataframe is created with DateID columns.
