...
the dimension and fact tables. This differs from Format 1 because the table is sorted by
GridID then DateID, as opposed to the other way around. The fact tables contain annual data
for multiple variables, alongisde the corresponding DateID and GridID values.
...
'FACT_YYYY.csv'           : | GridID (int) |     DateID (int)    |   Var1 (float)   |   ...    | VarN (float)|
...
OutputFolder = "D:\\CSIRO\\CSV FORMAT 4"
...
for Year in FileTable["Year"].drop_duplicates():
    ...
    for FileName in FileSeries:
        ...
        # reshape gridded data to ensure it is drilled down by location rather than date
        GriddedData = getattr( getattr(DS, ShortName), "values").flatten()
        GriddedData = GriddedData.reshape((DimTime, DimGrid))
        GriddedData = np.dstack(GriddedData).flatten()
        ...
        if FileName == FileSeries[0]:
            AnnualData = pd.DataFrame({
                "GridID": np.arange(1, DimGrid + 1).repeat(DimTime),
                "DateID": np.tile(np.arange(DateID_Start, DateID_End + 1), DimGrid),
                ShortName: GriddedData})
            
        # For all subsequent variables in the same year, a column is added.
        else:
            AnnualData[ShortName] = GriddedData
        DS.close()
