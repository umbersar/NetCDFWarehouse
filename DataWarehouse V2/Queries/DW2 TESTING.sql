...
FROM (
		(SELECT * FROM [DW1].[TIME2]
		WHERE [Year] = 1930 AND [Month] = 7 AND [Day] = 30) AS DW1_TIME  -- SPECIFY TIMESTEP HERE
...
FROM (
		(SELECT * FROM [DW1].[TIME2] WHERE [Year] = 1930) AS DW1_TIME  -- SPECIFY TEMPORAL RANGE HERE
