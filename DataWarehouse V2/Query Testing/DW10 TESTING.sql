...
PRINT 'QUERY TESTING FOR DataWarehouse Structure 10 (DW10): STAR-schema with a clustered columnstore, split integer date format.'
PRINT 'Re-using most elements of DW9 for simplicity'
PRINT 'COMMENT DW9 assembly took 2:52 hours'
PRINT 'COMMENT The FACT table occupies 59,608.641 MB, with 0 MB for the index.'
PRINT 'COMMENT The TIME table occupies 0.016 MB, with 0.016 MB for the index.'
PRINT 'COMMENT The GRID table occupies 2.281 MB, with 0 for the index.'
...
FROM (
		(SELECT * FROM [DW9].[TIME]
		WHERE [Year] = 1930 AND [Month] = 7 AND [Day] = 30) AS DW9_TIME  -- SPECIFY TIMESTEP HERE
		
		LEFT JOIN 
		[DW9].[FACT] AS DW9_FACT
		ON [DW9_TIME].[DateID] = [DW9_FACT].[DateID]

		LEFT JOIN 
		[DW9].[GRID] AS DW9_GRID
		ON [DW9_GRID].[GridID] = [DW9_FACT].[GridID]
		)
ORDER BY [DW9_FACT].[GridID]
...
FROM (
		(SELECT * FROM [DW9].[TIME] WHERE [Year] = 1930) AS DW9_TIME  -- SPECIFY TEMPORAL RANGE HERE
...
GROUP BY [DW9_FACT].[GridID], [Latitude], [Longitude]
ORDER BY [DW9_FACT].[GridID]
...
WITH [SAMPLE_PERIOD] AS(
	SELECT [Grid].[GridID], [Year], [Month], [E0]
	FROM (
		(SELECT * FROM [DW9].[TIME]
		WHERE [Year] BETWEEN 1911 AND 1920) AS [Time]  -- SPECIFY TEMPORAL RANGE HERE
...
WITH [SAMPLE_PERIOD] AS(
	...
	FROM (
		(SELECT * FROM [DW9].[TIME]
		WHERE [Year] BETWEEN 1911 AND 1930) AS [Time]  -- SPECIFY TEMPORAL RANGE HERE
...
WITH [SAMPLE_PERIOD] AS(
	...
	FROM (
		(SELECT * FROM [DW9].[TIME]
		WHERE [Year] BETWEEN 1911 AND 1940) AS [Time]  -- SPECIFY TEMPORAL RANGE HERE
...
WITH [SAMPLE_PERIOD] AS(
	...
	FROM (
		(SELECT * FROM [DW9].[TIME]
		WHERE [Year] BETWEEN 1911 AND 1950) AS [Time]  -- SPECIFY TEMPORAL RANGE HERE
