...
PRINT 'QUERY TESTING FOR DataWarehouse Structure 12 (DW12): STAR-schema v2 with a clustered cloumnstore index, split integer datetime format.'
PRINT 'COMMENT The OBT occupies 59,670.773 MB, with 0 MB for the index. Assembly took 05:15:28.'
PRINT 'COMMENT The GRID table occupies 2.281 MB, with 0 MB for the index.'
...
FROM (
		(SELECT * FROM [DW14].[OBT] 
		WHERE [Year] = 1930 AND [Month] = 7 AND [Day] = 30) AS DW14_FACT  -- SPECIFY TIMESTEP HERE
		LEFT JOIN 
		[DW14].[GRID] AS DW14_GRID
		ON [DW14_GRID].[GridID] = [DW14_FACT].[GridID]
		)
ORDER BY [DW14_FACT].[GridID]
...
FROM (
		(SELECT * FROM [DW14].[OBT] 
		WHERE [Year] = 1930) AS DW14_FACT  -- SPECIFY TEMPORAL RANGE HERE

		LEFT JOIN 
		[DW14].[GRID] AS DW14_GRID
		ON [DW14_GRID].[GridID] = [DW14_FACT].[GridID]
		)
GROUP BY [DW14_FACT].[GridID], [Latitude], [Longitude]
ORDER BY [DW14_FACT].[GridID]
...
FROM (
	...
	LEFT JOIN
	[DW14].[GRID] AS DW14_GRID
	ON [DW14_GRID].[GridID] = [DECILE].[GridID]
	)
WHERE [Year] = 1915 AND [Month] = 7 -- SPECIFY TIMESTEP TO BE QUERIED
ORDER BY [DW14_GRID].[GridID]
