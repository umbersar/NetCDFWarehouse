PRINT 'QUERY TESTING FOR DataWarehouse Structure 5 (DW5): STAR-schema v2 with a clustered rowstore index, datetime format.'
PRINT 'COMMENT The OBT occupies 266,833.109 MB, with 1,259.172 MB for the index. Assembly took 9 hours 53 minutes'
...
FROM (
		(SELECT * FROM [DW5].[OBT] WHERE [DateTime] = '19300730') AS DW5_FACT  -- SPECIFY TIMESTEP HERE
		LEFT JOIN 
		[DW5].[GRID] AS DW5_GRID
		ON [DW5_GRID].[GridID] = [DW5_FACT].[GridID]
		)
ORDER BY [DW5_FACT].[GridID]
...
FROM (
		(SELECT * FROM [DW5].[OBT] WHERE YEAR([DateTime]) = 1930) AS DW5_FACT  -- SPECIFY TEMPORAL RANGE HERE

		LEFT JOIN 
		[DW5].[GRID] AS DW5_GRID
		ON [DW5_GRID].[GridID] = [DW5_FACT].[GridID]
		)
GROUP BY [DW5_FACT].[GridID], [Latitude], [Longitude]
ORDER BY [DW5_FACT].[GridID]
