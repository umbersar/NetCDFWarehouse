PRINT 'QUERY TESTING FOR DataWarehouse Structure 5 (DW6): STAR-schema v2 with a clustered rowstore index, datetime format.'
...
FROM (
		(SELECT * FROM [DW6].[OBT] WHERE [DateTime] = '19300730') AS DW6_FACT  -- SPECIFY TIMESTEP HERE
		LEFT JOIN 
...
FROM (
		(SELECT * FROM [DW6].[OBT] WHERE YEAR([DateTime]) = 1930) AS DW6_FACT  -- SPECIFY TEMPORAL RANGE HERE
