PRINT 'QUERY TESTING FOR DataWarehouse Structure 9 (DW9): STAR-schema with a clustered columnstore, datetime format.'
...
FROM (
		(SELECT * FROM [DW9].[TIME] WHERE [DateTime] = '19300730') AS DW9_TIME  -- SPECIFY TIMESTEP HERE
...
FROM (
		(SELECT * FROM [DW9].[TIME] WHERE Year([DateTime]) = 1930) AS DW9_TIME  -- SPECIFY TEMPORAL RANGE HERE
