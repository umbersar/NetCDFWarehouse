...
WITH DECILES AS (
	SELECT  [SpaceID], [Year], [Month], 
		   (CASE WHEN [Total_E0] IS NOT NULL
				 THEN NTILE(10) OVER (PARTITION BY (CASE WHEN [Total_E0] IS NOT NULL THEN [SpaceID] ELSE NULL END)
									 ORDER BY [Total_E0]
									)
			END) as Decile
	FROM [OBT].[MONTHLY_V1]
...
FROM ((
	SELECT * FROM DECILES
	WHERE [Year] = 1911 AND [Month] = 1) AS S  -- SPECIFY TIMESTEP TO BE QUERIED
	JOIN
	[DIMENSION].[SPACE_OUTPUT] AS dim
	ON dim.[SpaceID] = S.[SpaceID])
ORDER BY dim.[SpaceID]

--SELECT TOP(10000) * FROM [OBT].[MONTHLY_V1] ORDER BY [SpaceID], [Year], [Month]
